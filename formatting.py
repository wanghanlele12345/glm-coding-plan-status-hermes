...
def format_reset_time(timestamp_ms: int) -> str:
    """Format next reset time as relative time string (e.g. '4h 30m')."""
...
def render_status_line(data: dict, model_name: str = "",
    ...
    token_part = f"Tokens: {progress_bar(token_pct)}"
    if reset_str:
        token_part += f" {C.DIM}(Resets in {reset_str}){C.RESET}"
    mcp_part = f"MCP: {mcp_pct}%"

    line1 = (
        f" {C.GRAY}\u2502{C.RESET} "
        f"{model_part} {C.GRAY}\u2502{C.RESET} "
        f"{token_part} {C.GRAY}\u2502{C.RESET} "
        f"{mcp_part} "
    )
